...
class Span {
  constructor(from: number, to:number, commit:Commit) {
    this.from = from; this.to = to; this.commit = commit
  }
}

class Commit {
  constructor(message, time, steps: ReplaceStep[], maps : StepMap[], hidden) {
...
class TrackState {
  ...
  constructor(blameMap: Span[], commits: Commit[], uncommittedSteps: Step[], uncommittedMaps: StepMap) {
  ...
  applyTransform(transform: Transform) {
    ...
    // the next commit
    let inverted =
      transform.steps.map((step, i) => step.invert(transform.docs[i])) as Step[]
